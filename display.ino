...
void initDisplay(){
    ...
    drawIndicator(2.7 * SCREEN_WIDTH / 8, SCREEN_MARGINS , TEXT_COLOR,"Current");
    drawIndicator(5.2 * SCREEN_WIDTH / 8, SCREEN_MARGINS , TEXT_COLOR,"Temperature");
}
...
void drawIndicator(int16_t x0, int16_t y0, uint16_t color, String title){
  ...
  lcd.Draw_Line(x1, y1, x1+r*cos(PI), y1+r*sin(PI));
  lcd.Set_Draw_color(DRAW_COLOR);
  if(currentState != System::DiodeSelecting){
    if(title == "Current")
